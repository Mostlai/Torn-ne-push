...
def main_handler():
	...
	if  misc['refills']['energy_refill_used'] == false:
		energy_refill += '未使用'
	else: energy_refill += '已使用'
	nerve_refill = 'N Refill: '
	if  misc['refills']['nerve_refill_used'] == false:
		nerve_refill += '未使用'
	else: nerve_refill += '已使用'
	token_refill = 'T Refill: '
	if  misc['refills']['token_refill_used'] == false:
		token_refill += '未使用'
	else: token_refill += '已使用'


    message = '{}\n{}\n能量: {}\n勇气: {}\n生命: {}\n{}\n{}\n{}\n{}\n{}\n{}\n{}\n{}'.format(
        name, status, energy, nerve, life, drug, booster, medical, more_info, energy_refill, nerve_refill, token_refill, events
    )
    QQPusher(message)
